...
m_DCSEC <- ulam(
  alist(
    ...
    #Set up participant change in SSRT over time
    #Week 1 (where wk2 and wk3 ar zero) plus changes for wk 2 and wk 3 AND stress
    mu_SSRT <- p_SSRT[part] + p_SSRTwk2[part]*Week2 + p_SSRTwk3[part]*Week3 + b_PSS*PSS + b_EmoWB*EmoWB + b_DCS*DCS,
...
m_AUCEC <- ulam(
  alist(
    ...
    #Set up participant change in SSRT over time
    #Week 1 (where wk2 and wk3 ar zero) plus changes for wk 2 and wk 3 AND stress
    mu_SSRT <- p_SSRT[part] + p_SSRTwk2[part]*Week2 + p_SSRTwk3[part]*Week3 + b_PSS*PSS + b_EmoWB*EmoWB + b_AUC*AUC,
...
# Save to CSV
write.csv(ECcomp_df, "Output/Expl_EC_Cort_ModelComp.csv", row.names = FALSE)
