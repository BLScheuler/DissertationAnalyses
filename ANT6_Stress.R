...
ItDf <- as.data.frame(ITmodData)

plot(ItDf$EmoWB, ItDf$IT)
summary(lm(IT~DCS, data= ItDf))

cor(as.matrix(ItDf[,c(4:7)]),use = "pairwise.complete.obs")
...
# Save to CSV
write.csv(ITcomp_df, "Output/Hyp3_IT_ModelComp.csv", row.names = FALSE)
